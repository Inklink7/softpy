    ...
    def fit(self, n_iters=10, keep_history=False, show_iters=False): 
        ...
        for it in range(n_iters+1):
            ...
            if keep_history:
             ...
             #dichiarazione lista condivisa

            q = np.empty(self.pop_size, dtype=self.candidate_type)
            if self.elitism:
                shared_q[:self.n_elite] = self.population[np.argsort(self.fitness)[::-1][:self.n_elite]]
            ...
            s1_time = time.time()
            processes = []
            for s in range(int((self.pop_size - sub)/2)): #eseguire in parallelo
                p = multiprocessing.Process(target=self.calculateGeneration, args=(shared_q, i))
                processes.append(p)
                p.start()
                i += 2

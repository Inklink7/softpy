...
class GeneticAlgorithm(MetaHeuristicsAlgorithm):
    ...
    def fit(self, n_iters=10, keep_history=False, show_iters=False): 
        ...
        for it in range(n_iters+1):
            ...
            for s in range(int((self.pop_size - sub)/2)): #eseguire in parallelo
                p = multiprocessing.Process(target=self.calculateGeneration, args=(result_queue, i))
                processes.append(p)
                p.start()
                i += 2
    ...
    def calculateGeneration(self, return_queue, index):
        ...
        return_queue.put((index, p1.recombine(p2).mutate()))
        return_queue.put((index + 1, p2.recombine(p1).mutate()))
